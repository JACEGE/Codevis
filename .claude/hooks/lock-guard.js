#!/usr/bin/env node
/**
 * CodeVis Lock Guard — PreToolUse Hook
            ...
            if (ladybug) {
                ...
                try {
                    ...
                    const result = await session.run(
                        `MATCH (f:Function {file: $file}) WHERE f.locked = true
                           AND (f.lockExpires IS NULL OR f.lockExpires > $now)
                         RETURN f.name AS name, f.lockedBy AS lockedBy, f.lockGroup AS lockGroup,
                                f.startLine AS startLine, f.endLine AS endLine`,
                        { file: relPath, now: Date.now() }
                    );
